...
Scrapes career pages of ~680 companies for backend SWE internships and new grad roles, targeting the **~May 2027 graduating class**. Filters by US-only locations, excludes PhD-required, senior, and frontend roles, and categorizes into:
- Summer 2027 Intern
...
The **entire Fall 2026 cycle is dropped**, intern and new grad alike — that recruiting season is over. 2027 is now the only in-range year.

**Summer 2027 is the only internship cycle targeted.** The Spring/Winter 2027 co-op term is not wanted, though *Spring 2027 New Grad* remains a valid new-grad cycle — the two are easy to confuse when editing `filters.py`.
...
- **Filters** (`filters.py`): Whole-word keyword matching for backend SWE roles (incl. software developer / SDE / architect titles); seniority exclusion (senior/staff/principal/lead/II/III/IV/L3+); firmware/embedded and cybersecurity exclusion; US location filter; PhD exclusion; and season/year categorization where the **title is authoritative** and description is only a tiebreaker. **Tuned recall-first** — see Key decisions: a year-less new-grad/intern role defaults to the 2027 target class rather than being dropped, and only clearly non-US locations are rejected.
...
- **Company config** (`companies.yaml`): ~680 entries. Each has `name`, `ats`, and either `board_id` or `url`.
...
- **A guessed board slug that returns 200 is not proof of identity — check whose board it is.** Adding companies by probing slug variants against the Greenhouse/Lever/Ashby/SmartRecruiters APIs finds live boards fast, but a live board is often a *different company with the same name*: `greenhouse/axiom` is a legal-services firm (not Axiom Space), `greenhouse/lattice` is the HR software company (not Lattice Semiconductor), `ashby/circle` is circle.so (not Circle/USDC), `ashby/cedar` is a home-services firm, `smartrecruiters/gong` is a journalism startup, and `greenhouse/linkedin` is a **test board** full of "Bug Bash Job" and "123123" rows. 13 of 143 verified-live boards were wrong this way — roughly one in eleven. Greenhouse and SmartRecruiters declare the board owner (`/v1/boards/{slug}` and `/v1/companies/{slug}` both return `name`); Ashby and Lever do not, so confirm those by reading actual job titles and locations. A stale board also announces itself in its postings — Lever's `anyscale` board holds a single row reading "We have moved our Careers Page to: jobs.ashbyhq.com/anyscale".
- **When slug guessing fails, read the careers page and follow the redirect.** Roughly 40% of candidate companies have no public JSON board — they are on Workday, Radancy, Phenom, Oracle or Eightfold, which this project drives by `url:` rather than `board_id:`. Fetching each company's own careers URL and grepping the final URL plus page HTML for a known ATS host resolved 27 of 87 such companies in one pass, and it is *stronger* evidence than slug guessing because the company itself is linking to the board. The same pass catches acquisitions, which are otherwise invisible: **careers.juniper.net now redirects to HPE's Workday and nianticlabs.com to careers.scopely.com** — both already in this file, so adding "Juniper" or "Niantic" would have silently scraped a company twice under two names.
- **Eightfold (`*.eightfold.ai`) has no scraper here yet.** NetApp, Lam Research and Millennium are all on it. That is the single highest-value missing platform if more coverage is wanted.
- **A board that returns 200 with zero jobs is still worth adding when provenance is good.** Kraken and Chronosphere have empty Ashby boards today, but their own careers pages link straight to those boards, so identity is confirmed and postings will be picked up when they appear. This is the one case where "0 jobs" is not a red flag.
- **Salary is never scraped.** The "$100k+ new grad" bar used to choose which companies are worth watching is editorial, based on widely reported US base pay — there is no salary field anywhere in the pipeline and none of it is verified. Treat it as a selection heuristic for the company list, not as data.
...
- **Firmware and embedded are excluded, and removing them from `INCLUDE_KEYWORDS` was not enough.** "Embedded Software Engineer Intern" and "Embedded Systems Engineer" still match `software engineer` / `systems engineer`, so they need an active rejection (`FIRMWARE_EMBEDDED_RE`) that out-ranks the include list, not just an absent keyword. The gate drops ~126 postings across the Greenhouse/Ashby/Lever boards. One carve-out: **"embedded finance / payments / banking" is a fintech product category, not a hardware role** — Column, Marqeta, Unit and Adyen all post backend engineering roles using the phrase — so `\bembedded\b` is negative-lookahead'd against those words.
- **Retiring an *intern* season has the same trap, in a different place.** `_intern_category()` ends with `return "Summer 2027 Intern"` as its recall-first default, so a season that is merely absent from `_INTERN_CYCLES` gets silently **relabelled Summer** rather than dropped. Removing the Spring/Winter 2027 co-op cycle therefore needed an explicit `_DROPPED_INTERN_SEASONS = {"spring"}` check placed *before* that default. (`_season_from()` folds `winter` into `spring`, so one entry covers both.) Note the season is read from title **or description**, so a season-less title like Point72's "Quantitative Developer Intern" is dropped when its description clearly describes a spring term — that is intended. A posting naming *two* seasons ("Spring/Summer 2027", "Summer or Winter") is kept as Summer, because `_season_from()` returns None on an ambiguous match and the Summer default is correct for a role that genuinely offers both.
- **Cybersecurity exclusion lives in `EXCLUDE_KEYWORDS`, not a bespoke regex — deliberately.** `_EXCLUDE_RE` is checked before `_INCLUDE_RE`, already does whole-word matching, and **`audit_scrape.py` reads the same list**, so unlike the firmware gate this one cannot drift out of step with the audit. Prefer this route for any future role-type exclusion that doesn't need a lookahead. The workhorse term is plain `security` — it covers "Security Engineer", "Product Security", "Information Security" and "Software Engineer, Security" in one. Whole-word matching is what keeps it clear of **"Securities"** (Citadel Securities, Maven Securities, "Software Engineer - Securities Finance"), which is a different word; a stemmed `securit\w*` would have silently gutted the quant bucket.
- **Retiring a cycle means moving it to the *past-cycle* list, not just deleting its bucket.** `categorize()` is recall-first: a new-grad role whose season maps to nothing falls through to the season-less 2027 default. So dropping Fall 2026 required `_past_cycle_in_title()` to start matching `fall|autumn 2026` in the same change — otherwise every Fall 2026 posting would have been silently *relabelled* "2027 New Grad" instead of dropped, which is worse than leaving the bucket alone. `_NEWGRAD_CYCLES` and `_past_cycle_in_title()` must always partition the seasons between them. The same edit collapsed `db.py`'s `NON_TARGET_TITLE_CYCLE_RE` to a single range (`2010-2026, 2028+`).
- **`scripts/audit_scrape.py` re-implements the filter chain, so every new gate has to be added there too.** It deliberately replays the chain gate-by-gate to report *which* gate dropped a posting — which means it silently drifts from reality when a gate is added to `filters.py` only. The firmware/embedded change hit exactly this: the audit went on reporting embedded interns as kept while the real scraper dropped them. If the audit and `matches_backend_swe()` ever disagree, the audit is wrong.
